...
def get_reg_loss(cls_score, mask_score, pred_reg, reg_label, loc_scope, loc_bin_size, num_head_bin, anchor_size,
    ...
    if cfg.TRAIN.IOU_LOSS_TYPE == 'raw':
        ...
        insect_x = torch.min((pred_x + pred_size[:, 2]/2), (tar_x + tar_size[:, 2]/2)) - torch.max((pred_x - pred_size[:, 2]/2), (tar_x - tar_size[:, 2]/2))
        insect_y = torch.min((pred_y + pred_size[:, 0]/2), (tar_y + tar_size[:, 0]/2)) - torch.max((pred_y - pred_size[:, 0]/2), (tar_y - tar_size[:, 0]/2))
        insect_z = torch.min((pred_z + pred_size[:, 1]/2), (tar_z + tar_size[:, 1]/2)) - torch.max((pred_z - pred_size[:, 1]/2), (tar_z - tar_size[:, 1]/2))

        closure_x = torch.max((pred_x + pred_size[:, 2]/2), (tar_x + tar_size[:, 2]/2)) - torch.min((pred_x - pred_size[:, 2]/2), (tar_x - tar_size[:, 2]/2))
        closure_y = torch.max((pred_y + pred_size[:, 0]/2), (tar_y + tar_size[:, 0]/2)) - torch.min((pred_y - pred_size[:, 0]/2), (tar_y - tar_size[:, 0]/2))
        closure_z = torch.max((pred_z + pred_size[:, 1]/2), (tar_z + tar_size[:, 1]/2)) - torch.min((pred_z - pred_size[:, 1]/2), (tar_z - tar_size[:, 1]/2))
        ...
        tar_area = tar_size[:, 0] * tar_size[:, 1] * tar_size[:, 2]
        iou_tmp = torch.max(insect_area/(pred_area+tar_area-insect_area) - (closure_area - (pred_area+tar_area-insect_area)) / closure_area, pred_x.new().resize_(pred_x.shape).fill_(1e-3))
